    ...
    internal class GenerateUnitOfWork
    {
        public static string Generate(Type type, string name_space, int selectedIndex)
        {
            if (selectedIndex == 0)
            {
                ...
                $"{GeneralClass.newlinepad(8)}public async Task<Either<GeneralFailure, int>> CommitAllChanges(CancellationToken cancellationToken)=>throw new NotImplementedException(\"Its not been used to commit for now individual repo implemented savechanges\"); " +
